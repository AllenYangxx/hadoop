...
public class WordCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //DfsTest.copyFileToHDFS("E:\\大数据\\history.json","/user/dfstest/history.json");
        FileInputFormat.addInputPath(job, new Path("/user/dfstest/history.json")); // 设置输入文件
        FileOutputFormat.setOutputPath(job, new Path("/user/dfstest/output5")); // 设置输出文件
        boolean success = job.waitForCompletion(true);
        DfsTest.readFile("/user/dfstest/output5/part-r-00000");
        System.exit(success ? 0 : 1); // 等待完成退出
    }
}
